...
// Separado fora da função anterior para evitar erro de sintaxe
...
export const parseHeadToHeadCSV = (csvText: string): HeadToHeadMatch[] => {
  if (!csvText || csvText.trim() === '') {
    console.warn('Head to Head CSV text is empty or invalid.');
    return [];
  }
  const lines = csvText.trim().split('\n');
  if (lines.length < 2) {
    console.warn('Head to Head CSV has no data rows.');
    return [];
  }
  const headers = lines[0].split(',');
  ...
  return lines.slice(1).map(line => {
    ...
    return {
      ...
      Team_Home: cols[headers.indexOf('Home')]?.trim() || '',
      Team_Away: cols[headers.indexOf('Away')]?.trim() || '',
      Goals_Home: Number(cols[headers.indexOf('Gols_Home')] || 0),
      Goals_Away: Number(cols[headers.indexOf('Gols_Away')] || 0),
      Result: cols[headers.indexOf('Resultado')]?.trim() || '',
    };
  }).filter(match => match.Team_Home && match.Team_Away);
};
